...
public class ContainerFilter extends Container {
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slot) {
            ...
            if (slotObject != null && slotObject.getHasStack()) {
                    ...
                    if (slot < tileEntity.getSizeInventory()+6) {
                            if (!this.mergeItemStack(stackInSlot, tileEntity.getSizeInventory()+6, 42+tileEntity.getSizeInventory(), true)) {
                                    return null;
                            }
                    }
                    //places it into the tileEntity is possible since its in the player inventory
                    else if (!this.mergeItemStack(stackInSlot, 0, tileEntity.getSizeInventory()+6, false)) {
                            return null;
                    }
