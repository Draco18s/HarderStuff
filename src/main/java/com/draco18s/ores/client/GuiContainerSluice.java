...
public class GuiContainerSluice extends GuiContainer {
	TileEntitySluice te;
	
	public GuiContainerSluice(Container par1Container) {
		super(par1Container);
	}

	public GuiContainerSluice (InventoryPlayer inventoryPlayer, TileEntitySluice tileEntity) {
		//the container is instanciated and passed to the superclass for handling
		super(new ContainerSluice(inventoryPlayer, tileEntity));
		te = tileEntity;
	}
	...
	protected void drawGuiContainerForegroundLayer(int param1, int param2) {
		...
		String str = "" + (te.getGravel() + te.getSand());
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
		...
		int progress = Math.round(19 * (75 - te.getTime()) / 75.0f);
		if(te.getTime() == 0) {
			progress = 0;
		}
		this.drawTexturedModalRect(x+88, y+56, 176, 0, progress, 8);
		progress = 0;
		int d = te.getGravel()+te.getSand()+te.getDirt();
		if(d <= 320) {
