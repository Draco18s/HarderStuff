...
public class TileEntitySifter extends TileEntity implements ISidedInventory {
	...
	private void siftItem() {
        ...
        if (canSift(0)) {
    		ItemStack itemstack = HardLibAPI.recipeManager.getSiftResult(inventory[0], true);
            ...
            inventory[0].stackSize -= HardLibAPI.recipeManager.getSiftAmount(inventory[0]);
            //sifted = true;
        	...
        	ItemStack itemstack = HardLibAPI.recipeManager.getSiftResult(inventory[1], true);
            ...
            inventory[1].stackSize -= HardLibAPI.recipeManager.getSiftAmount(inventory[1]);
	...
	private void suckItems() {
		...
		if(ents.size() > 0) {
			...
			for(int e = ents.size()-1; e >= 0; e--) {
				...
				if(HardLibAPI.recipeManager.getSiftResult(stack, false) != null) {
	...
	private boolean canSift(int a) {
		if (inventory[a] == null) {
            ...
            ItemStack itemstack = HardLibAPI.recipeManager.getSiftResult(inventory[a], true);
