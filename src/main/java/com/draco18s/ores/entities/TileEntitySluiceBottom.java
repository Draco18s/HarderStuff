...
public class TileEntitySluiceBottom extends TileEntity implements ISidedInventory {
	...
	private void makeRequest() {
		...
		if(te != null && te instanceof TileEntitySluiceBottom && te != this) {
	...
	private void suckItems() {
		...
		//System.out.println((worldObj.isRemote?"Client":"Server") + ":" + ents.size() + " entities");
		EntityItem ent;
		if(ents.size() > 0) {
			ItemStack stack;
	...
	public boolean checkWater() {
		...
		do {
			...
			if(worldObj.getBlock(xCoord+x, yCoord, zCoord+z) == Blocks.water || worldObj.getBlock(xCoord+x, yCoord, zCoord+z) == Blocks.flowing_water) {
				...
				if(calcflowItemsTowards) {
					Vec3 vec3 = this.getFlowVector(worldObj, xCoord+x, yCoord, zCoord+z);
					double v = vec3.xCoord == 0.0D && vec3.zCoord == 0.0D ? -1000.0D : Math.atan2(vec3.zCoord, vec3.xCoord) - (Math.PI / 2D);
		...
		if(!worldObj.isRemote) {
			if(ret && worldObj.getBlock(xCoord-xx, yCoord-1, zCoord-zz) == Blocks.air  && OresBase.sluicePlacesWaterSource) {
	...
	private void doFilter() {
		...
		Block b = HardLibAPI.recipeManager.getRandomSluiceResult(this.rand, inventory[1].getItem());
	...
	public int[] getAccessibleSlotsFromSide(int side) {
		if(this.yCoord <= 0 || (side == 1 && flowItemsTowards && (downstremrequests > 0 || inventory[1] == null)))
			return new int[] {0};
		else
			return new int[] {};
	}
	...
	public boolean canInsertItem(int slot, ItemStack stack, int side) {
		boolean flag = downstremrequests > 0;
		//if(inventory[1] != null)
			//downstremrequests--;
		return (side==1&&slot==0)&&(inventory[1] == null || flag);
	}
	...
	private Vec3 getFlowVector(IBlockAccess world, int x, int y, int z) {
        ...
        int l = this.getEffectiveFlowDecay(world, x, y, z);
        ...
        for (int i1 = 0; i1 < 4; ++i1) {
            ...
            int l1 = this.getEffectiveFlowDecay(world, j1, y, k1);
            int i2;

            if (l1 < 0) {
                if (!world.getBlock(j1, y, k1).getMaterial().blocksMovement()) {
                    l1 = this.getEffectiveFlowDecay(world, j1, y - 1, k1);
        ...
        if (world.getBlockMetadata(x, y, z) >= 8) {
            ...
            if (flag || Blocks.water.isBlockSolid(world, x, y, z - 1, 2)) {
                flag = true;
            }

            if (flag || Blocks.water.isBlockSolid(world, x, y, z + 1, 3)) {
                flag = true;
            }

            if (flag || Blocks.water.isBlockSolid(world, x - 1, y, z, 4)) {
                flag = true;
            }

            if (flag || Blocks.water.isBlockSolid(world, x + 1, y, z, 5)) {
                flag = true;
            }

            if (flag || Blocks.water.isBlockSolid(world, x, y + 1, z - 1, 2)) {
                flag = true;
            }

            if (flag || Blocks.water.isBlockSolid(world, x, y + 1, z + 1, 3)) {
                flag = true;
            }

            if (flag || Blocks.water.isBlockSolid(world, x - 1, y + 1, z, 4)) {
                flag = true;
            }

            if (flag || Blocks.water.isBlockSolid(world, x + 1, y + 1, z, 5)) {
                flag = true;
            }
	...
	private int getEffectiveFlowDecay(IBlockAccess world, int x, int y, int z) {
