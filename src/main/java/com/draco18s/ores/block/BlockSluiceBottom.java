...
public class BlockSluiceBottom extends BlockContainer {
	...
	public BlockSluiceBottom() {
		super(OresBase.materialSluiceBox);
		setBlockName("sluice");
		setHardness(2.0f);
    ...
    private void setMeta(World world, int x, int y, int z) {
        if (!world.isRemote) {
            ...
            if((b0 == 0 || b0 == 1) && (!block3.isNormalCube() || !block2.isNormalCube() || m2 == Material.ground || m3 == Material.ground || m2 == Material.grass || m3 == Material.grass || m2 == Material.sand || m3 == Material.sand)) {
            	b0 += 7;
            }
            if((b0 == 2 || b0 == 3) && (!block1.isNormalCube() || !block.isNormalCube() || m  == Material.ground || m1 == Material.ground || m  == Material.grass || m1 == Material.grass || m == Material.sand || m1 == Material.sand)) {
            	b0 += 7;
            }
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
		TileEntitySluiceBottom te = (TileEntitySluiceBottom) world.getTileEntity(x, y, z);
        if (te != null && te.getWaterAmount() > 0 && te.getTime() <= 0) {
	...
	private void sparkle(World world, int x, int y, int z) {
		...
		for (int l = 0; l < 5; ++l) {
            double d1 = (double)((float)x + random.nextFloat()/2 + 0.25);
            double d2 = (double)((float)y + random.nextFloat());
            double d3 = (double)((float)z + random.nextFloat()/2 + 0.25);
