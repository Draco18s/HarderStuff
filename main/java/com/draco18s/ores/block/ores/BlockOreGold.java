...
public class BlockOreGold extends Block {
	private IIcon[] icons;
	private boolean avoidGeneration = false;
	
	public BlockOreGold() {
		super(Material.rock);
		setBlockName("ore_gold");
		setHardness(9.0f);
		setHarvestLevel("pickaxe", 1);
		setResistance(5.0f);
        setCreativeTab(CreativeTabs.tabBlock);
        this.setBlockTextureName("gold_ore");
	}
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
		icons = new IIcon[16];
		for(int i=0; i < 16; i++) {
			icons[i] = iconRegister.registerIcon("ores:gold_ore_"+i);
		}
    }
	...
	public void onBlockDestroyedByPlayer(World world, int x, int y, int z, int m) {
		if(m > 0) {
			world.setBlock(x, y, z, this, m-1, 3);
		}
	...
	public void onBlockAdded(World world, int x, int y, int z) {
		...
		if(m < 9 && rand.nextInt(200) == 0) {
			for(int j=1; y+j < 90; j++) {
				if(world.getBlock(x, y+j, z) == Blocks.grass && world.getBlock(x, y+j+1, z) == Blocks.air) {
					OresBase.scatterFlowers(world, x, y+j+1, z, OresBase.blockOreFlowers, 1, 20, 9, 11);
					//OresBase.scatterFlowers(world, x, y+j+1, z, OresBase.blockOreFlowers, 2, 20, 2, 11);
					//world.setBlock(x, y+j+1, z, OresBase.blockHorsetail);
					return;
				}
			}
		}
		//if(OresBase.rand.nextInt(10) == 0 ) {
		//	OreDataHooks.saveOreData(world, x, y, z, this, (m+1)*10);
		//}
	}
}
