...
public class BlockOreLead extends Block {
	...
	public BlockOreLead() {
		super(Material.rock);
		setBlockName("ore_lead");
		setHardness(6.0f);
		setHarvestLevel("pickaxe", 1);
		setResistance(5.0f);
        setCreativeTab(CreativeTabs.tabBlock);
        this.setBlockTextureName("lead_ore");
	}
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
		icons = new IIcon[16];
		for(int i=0; i < 16; i++) {
			icons[i] = iconRegister.registerIcon("ores:ic2/lead_ore_"+i);
		}
    }
		...
		if(m < 9 && rand.nextInt(200) == 0) {
			for(int j=1; y+j < 90; j++) {
				if(world.getBlock(x, y+j, z) == Blocks.grass && world.getBlock(x, y+j+1, z) == Blocks.air) {
					OresBase.scatterFlowers(world, x, y+j+1, z, OresBase.blockOreFlowers, 9, 20, 4, 7);
					return;
				}
			}
		}
