...
public class BlockOreCopper extends Block {
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
		icons = new IIcon[16];
		for(int i=0; i < 16; i++) {
			icons[i] = iconRegister.registerIcon("ores:ic2/copper_ore_"+i);
		}
    }
	...
	public void onBlockAdded(World world, int x, int y, int z) {
		...
		if(m < 9 && rand.nextInt(300) == 0) {
			for(int j=1; y+j < 90; j++) {
				if(world.getBlock(x, y+j, z) == Blocks.grass && world.getBlock(x, y+j+1, z) == Blocks.air) {
					OresBase.scatterFlowers(world, x, y+j+1, z, OresBase.blockOreFlowers, 8, 20, 4, 9);
					return;
				}
			}
		}
