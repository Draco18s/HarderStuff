...
public class BlockOreUranium extends Block {
	...
	public BlockOreUranium() {
        ...
        this.setBlockTextureName("uranium_ore");
	}
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
		icons = new IIcon[16];
		for(int i=0; i < 16; i++) {
			icons[i] = iconRegister.registerIcon("ores:ic2/uranium_ore_"+i);
		}
    }
		...
		if(m < 9 && rand.nextInt(400) == 0) {
			for(int j=1; y+j < 90; j++) {
				if(world.getBlock(x, y+j, z) == Blocks.grass && world.getBlock(x, y+j+1, z) == Blocks.air) {
					OresBase.scatterFlowers(world, x, y+j+1, z, OresBase.blockOreFlowers, 10, 30, 2, 7);
					return;
				}
			}
		}
