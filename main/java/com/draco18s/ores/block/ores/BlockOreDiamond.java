...
public class BlockOreDiamond extends Block {
	private IIcon[] icons;
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
		icons = new IIcon[16];
		for(int i=0; i < 16; i++) {
			icons[i] = iconRegister.registerIcon("ores:diamond_ore_"+i);
		}
    }
	...
	public void onBlockDestroyedByPlayer(World world, int x, int y, int z, int m) {
		if(m > 2) {
			world.setBlock(x, y, z, this, m-3, 3);
		}
	...
	public void dropBlockAsItemWithChance(World world, int x, int y, int z, int meta, float chance, int fortune) {
        if (!world.isRemote) {
            ...
            if(world.getBlock(x, y, z) != this && meta > 2) {
            	//avoidGeneration = true;
    			world.setBlock(x, y, z, this, meta-3, 3);
            }
            /*else {
            	//mining laser hack
            	for(int i = meta-1; i >= 2; i-=3) {
	...
	public void onBlockAdded(World world, int x, int y, int z) {
		...
		if(m < 9 && rand.nextInt(400) == 0) {
			for(int j=1; y+j < 90; j++) {
				if(world.getBlock(x, y+j, z) == Blocks.grass && world.getBlock(x, y+j+1, z) == Blocks.air) {
					OresBase.scatterFlowers(world, x, y+j+1, z, OresBase.blockOreFlowers, 3, 20, 8, 11);
					return;
				}
			}
		}
		//if(OresBase.rand.nextInt(10) == 0 ) {
		//	OreDataHooks.saveOreData(world, x, y, z, this, m+1);
		//}
	}
}
