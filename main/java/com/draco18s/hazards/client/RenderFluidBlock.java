 ...
 * 
 */
public class RenderFluidBlock implements ISimpleBlockRenderingHandler
{
    public static RenderFluidBlock instance = new RenderFluidBlock();
    ...
    {
    	int denDir = block.getFluid().getDensity() > 0 ? -1 : 1;
        if (world.getBlock(x, y, z) == block)
        {
            if (world.getBlock(x, y - denDir, z) == block)
            {
                return 1;
            }
        ...
        return !world.getBlock(x, y, z).getMaterial().isSolid() && world.getBlock(x, y - denDir, z) == block ? 1 : block.getQuantaPercentage(world, x, y, z) * 0.875F;
    }
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer)
    {
        ...
        int denDir = ((BlockFluidBase) block).getFluid().getDensity() > 0 ? -1 : 1;
        ...
        boolean renderTop = world.getBlock(x, y - denDir, z) != theFluid;

        boolean renderBottom = block.shouldSideBeRendered(world, x, y + denDir, z, 0) && world.getBlock(x, y + denDir, z) != theFluid;
